...
# Module: Training
# Purpose:
#   - Load train/val/test token streams
#   - Train decoder-only transformer on PMC-prepared data
#   - Use AdamW + warmup/cosine decay
#   - Use AMP + gradient accumulation + gradient clipping
#   - Report loss and perplexity
#   - Save checkpoints
# ---------------------------------------------------------------------
...
def main():
    ...
    best_ckpt_path = CHECKPOINT_DIR / "ckpt.pt"
    final_ckpt_path = CHECKPOINT_DIR / "cat_final.pt"
    ...
    for step in range(max_iters + 1):
        ...
        if step % eval_interval == 0:
            ...
            if metrics["val_loss"] < best_val_loss:
                ...
                save_checkpoint(
                    best_ckpt_path, model, optimizer, step, best_val_loss, config
                )
    ...
    save_checkpoint(final_ckpt_path, model, optimizer, max_iters, best_val_loss, config)
